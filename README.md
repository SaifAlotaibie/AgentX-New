...
### كيف يعمل النظام مع المستخدم؟ (مثال عملي)

```mermaid
sequenceDiagram
    autonumber
    participant User as 👤 سيف المستخدم
    participant Agent as 🤖 قوى المساعد الذكي
    participant DB as 💾 قاعدة البيانات
    
    Note over User,DB: مثال: تحديث السيرة الذاتية
    
    User->>Agent: ابي احدث سنوات خبرتي تصير 10 سنوات
    
    Note over Agent: يفكر و يحلل الطلب
    Agent->>Agent: 1️⃣ تحليل: المستخدم يريد تحديث السيرة
    Agent->>Agent: 2️⃣ قرار: احتاج اجلب السيرة الحالية
    
    Agent->>DB: جلب السيرة الذاتية الحالية
    DB-->>Agent: سنوات الخبرة الحالية: 5 سنوات
    
    Note over Agent: يفكر بالخطوة التالية
    Agent->>Agent: 3️⃣ قرار: احتاج اعدل سنوات الخبرة
    
    Agent->>DB: تحديث سنوات الخبرة إلى 10
    DB-->>Agent: تم التحديث بنجاح ✅
    
    Agent->>DB: حفظ سجل العملية
    DB-->>Agent: تم الحفظ
    
    Note over Agent: ينشئ الرد النهائي
