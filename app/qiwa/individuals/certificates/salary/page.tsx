...
export default function SalaryCertificatePage() {
  ...
  const fetchCertificates = async () => {
    try {
      ...
      if (result.success) {
        const salaryCerts = (result.data || []).filter((cert: Certificate) => cert.certificate_type === 'salary_definition')
        setCertificates(salaryCerts)
      } else {
  ...
  const handleGenerate = async () => {
    try {
      ...
      if (result.success) {
        ...
        alert(result.error || 'فشل في إصدار الشهادة')
      }
    } catch (err) {
    ...
    <div className="min-h-screen" style={{ backgroundColor: '#fafafa' }}>
      ...
      <div className="container mx-auto px-6 py-12">
        <Breadcrumb items={[
          ...
          { label: 'شهادة التعريف بالراتب' }
        ]} />

        <div className="max-w-4xl mx-auto">
          <div className="bg-gradient-to-br from-green-500 to-green-600 text-white rounded-3xl p-12 mb-8 shadow-2xl">
            <div className="flex items-center gap-4 mb-4">
              <div className="w-20 h-20 bg-white/20 rounded-2xl flex items-center justify-center text-5xl backdrop-blur-sm">
                💰
              </div>
              <div>
                <h1 className="text-4xl font-bold">شهادة التعريف بالراتب</h1>
                <p className="text-white/80 text-lg mt-2">إصدار شهادة راتب رسمية</p>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white rounded-2xl shadow-lg p-8 mb-8">
            ...
            <p className="leading-relaxed mb-6" style={{ color: '#4b515a' }}>
              احصل على شهادة رسمية تثبت راتبك الشهري ووظيفتك الحالية. يمكن استخدام هذه الشهادة للبنوك، التمويل، أو أي إجراءات رسمية أخرى.
            </p>

            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
              <div className="border-2 rounded-xl p-6" style={{ borderColor: '#e0e0e0' }}>
                <div className="text-3xl mb-3">⚡</div>
                <h3 className="font-bold mb-2" style={{ color: '#20183b' }}>مدة الإنجاز</h3>
                <p style={{ color: '#4b515a' }}>فوري</p>
              </div>
              <div className="border-2 rounded-xl p-6" style={{ borderColor: '#e0e0e0' }}>
                <div className="text-3xl mb-3">✅</div>
                <h3 className="font-bold mb-2" style={{ color: '#20183b' }}>معتمدة</h3>
                <p style={{ color: '#4b515a' }}>معتمدة من الوزارة</p>
              </div>
            </div>

            <div className="bg-green-50 border-2 border-green-200 rounded-xl p-6 mb-6">
              <h3 className="font-bold mb-3 text-green-900">المتطلبات</h3>
              <ul className="space-y-2 text-green-800">
                <li className="flex items-start gap-2">
                  <span className="text-xl">✓</span>
                  <span>وجود عقد عمل نشط</span>
                </li>
                <li className="flex items-start gap-2">
                  <span className="text-xl">✓</span>
                  <span>تسجيل بيانات الراتب في النظام</span>
                </li>
              </ul>
            </div>
          ...
          {certificates.length > 0 && (
            <div className="bg-white rounded-2xl shadow-lg p-8">
              <h3 className="text-xl font-bold mb-6" style={{ color: '#20183b' }}>الشهادات الصادرة</h3>
